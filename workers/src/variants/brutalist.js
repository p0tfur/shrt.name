/**
 * BRUTALIST VARIANT HTML
 * Anti-design, brutalist aesthetic, uppercase, raw elements
 */

export const brutalistHTML = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>SHRT.NAME // BRUTALIST URL SHORTENER</title>
  <meta name="description" content="BRUTALIST URL SHORTENER - NO DESIGN - JUST FUNCTION">
  <style>
    :root {
      --black: #000000;
      --white: #ffffff;
      --red: #ff0000;
      --yellow: #ffff00;
      --blue: #0000ff;
      --font-display: 'Impact', 'Haettenschweiler', 'Arial Narrow Bold', sans-serif;
      --font-mono: 'Courier New', 'Courier', monospace;
      --space-xs: 4px;
      --space-sm: 8px;
      --space-md: 16px;
      --space-lg: 32px;
      --space-xl: 64px;
      --border-thick: 4px solid var(--black);
      --border-thin: 2px solid var(--black);
      --shadow-hard: 8px 8px 0px var(--black);
      --shadow-offset: 4px 4px 0px var(--black);
    }
    *, *::before, *::after {
      margin: 0;
      padding: 0;
      box-sizing: border-box;
    }
    body {
      font-family: var(--font-mono);
      font-weight: 400;
      background: var(--white);
      color: var(--black);
      min-height: 100vh;
      padding: var(--space-md);
      line-height: 1.4;
      letter-spacing: -0.02em;
      position: relative;
      overflow-x: hidden;
    }
    body::before {
      content: '';
      position: fixed;
      top: 0;
      left: 0;
      width: 100%;
      height: 100%;
      background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 400 400' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E");
      opacity: 0.15;
      pointer-events: none;
      z-index: 1000;
    }
    .container {
      max-width: 1000px;
      margin: 0 auto;
      position: relative;
    }
    .header {
      border: var(--border-thick);
      background: var(--black);
      color: var(--white);
      padding: var(--space-lg) var(--space-md);
      margin-bottom: var(--space-lg);
      text-transform: uppercase;
    }
    .header h1 {
      font-family: var(--font-display);
      font-size: clamp(2rem, 8vw, 5rem);
      font-weight: normal;
      letter-spacing: 0.05em;
      margin-bottom: var(--space-sm);
      text-shadow: var(--shadow-offset);
      transform: rotate(-1deg);
    }
    .header p {
      font-size: 1.1rem;
      font-weight: 700;
      letter-spacing: 0.1em;
      opacity: 0.9;
    }
    .main {
      border: var(--border-thick);
      padding: var(--space-lg) var(--space-md);
      margin-bottom: var(--space-lg);
      background: var(--white);
    }
    .section-title {
      font-family: var(--font-display);
      font-size: clamp(1.5rem, 5vw, 2.5rem);
      text-transform: uppercase;
      margin-bottom: var(--space-md);
      padding-bottom: var(--space-sm);
      border-bottom: var(--border-thick);
      letter-spacing: 0.05em;
    }
    .input-group {
      margin-bottom: var(--space-lg);
    }
    .input-label {
      display: block;
      font-family: var(--font-display);
      font-size: 1.5rem;
      text-transform: uppercase;
      margin-bottom: var(--space-sm);
      font-weight: normal;
    }
    .url-input {
      width: 100%;
      font-family: var(--font-mono);
      font-size: 1.1rem;
      font-weight: 700;
      padding: var(--space-md);
      border: var(--border-thick);
      background: var(--white);
      color: var(--black);
      text-transform: uppercase;
      outline: none;
      transition: all 0.1s ease;
    }
    .url-input:focus {
      background: var(--black);
      color: var(--white);
      box-shadow: var(--shadow-hard);
      transform: translate(-4px, -4px);
    }
    .url-input::placeholder {
      text-transform: uppercase;
      opacity: 0.5;
    }
    .custom-code-group {
      margin-top: var(--space-md);
    }
    .custom-code-label {
      display: block;
      font-family: var(--font-display);
      font-size: 1.3rem;
      text-transform: uppercase;
      margin-bottom: var(--space-sm);
      font-weight: normal;
      opacity: 0.7;
    }
    .custom-code-input {
      width: 100%;
      font-family: var(--font-mono);
      font-size: 1rem;
      font-weight: 700;
      padding: var(--space-sm);
      border: var(--border-thin);
      background: var(--white);
      color: var(--black);
      text-transform: uppercase;
      outline: none;
    }
    .custom-code-input:focus {
      border-width: var(--space-xs);
      background: var(--black);
      color: var(--white);
    }
    .custom-code-input::placeholder {
      text-transform: uppercase;
      opacity: 0.3;
    }
    .submit-btn {
      width: 100%;
      font-family: var(--font-display);
      font-size: clamp(1.5rem, 4vw, 2rem);
      font-weight: normal;
      text-transform: uppercase;
      padding: var(--space-md);
      margin-top: var(--space-lg);
      border: var(--border-thick);
      background: var(--black);
      color: var(--white);
      cursor: pointer;
      letter-spacing: 0.1em;
      transition: all 0.1s ease;
      box-shadow: var(--shadow-hard);
      transform: translate(-4px, -4px);
    }
    .submit-btn:hover {
      background: var(--red);
      transform: translate(-8px, -8px);
      box-shadow: 12px 12px 0px var(--black);
    }
    .submit-btn:active {
      transform: translate(0, 0);
      box-shadow: 4px 4px 0px var(--black);
    }
    .submit-btn:disabled {
      background: var(--black);
      opacity: 0.5;
      cursor: not-allowed;
      transform: translate(0, 0);
      box-shadow: none;
    }
    .loading {
      text-align: center;
      font-family: var(--font-mono);
      font-size: 1.3rem;
      font-weight: 700;
      text-transform: uppercase;
      padding: var(--space-lg);
      background: var(--yellow);
      color: var(--black);
      border: var(--border-thick);
      margin-top: var(--space-lg);
      animation: blink 0.3s infinite;
    }
    @keyframes blink {
      0%, 100% { opacity: 1; }
      50% { opacity: 0.5; }
    }
    .result {
      margin-top: var(--space-lg);
      border: var(--border-thick);
      padding: var(--space-lg) var(--space-md);
      background: var(--yellow);
      display: none;
      box-shadow: var(--shadow-hard);
      transform: translate(-4px, -4px);
    }
    .result.show {
      display: block;
      animation: slideIn 0.3s ease-out;
    }
    @keyframes slideIn {
      from { opacity: 0; transform: translateY(20px); }
      to { opacity: 1; transform: translateY(0); }
    }
    .result-title {
      font-family: var(--font-display);
      font-size: 2rem;
      text-transform: uppercase;
      margin-bottom: var(--space-md);
      padding-bottom: var(--space-sm);
      border-bottom: var(--border-thick);
    }
    .short-url {
      font-family: var(--font-mono);
      font-size: 1.5rem;
      font-weight: 700;
      color: var(--black);
      text-transform: uppercase;
      margin-bottom: var(--space-md);
      word-break: break-all;
      background: var(--black);
      color: var(--white);
      padding: var(--space-sm);
      display: inline-block;
      border: var(--border-thin);
    }
    .original-url {
      font-family: var(--font-mono);
      font-size: 0.9rem;
      font-weight: 400;
      color: var(--black);
      text-transform: uppercase;
      margin-bottom: var(--space-md);
      word-break: break-all;
      opacity: 0.7;
    }
    .result-actions {
      display: flex;
      gap: var(--space-md);
      flex-wrap: wrap;
    }
    .copy-btn, .visit-btn {
      flex: 1;
      min-width: 200px;
      font-family: var(--font-display);
      font-size: 1.2rem;
      font-weight: normal;
      text-transform: uppercase;
      padding: var(--space-sm) var(--space-md);
      border: var(--border-thick);
      background: var(--white);
      color: var(--black);
      cursor: pointer;
      letter-spacing: 0.05em;
      transition: all 0.1s ease;
    }
    .copy-btn:hover, .visit-btn:hover {
      background: var(--black);
      color: var(--white);
      box-shadow: var(--shadow-offset);
      transform: translate(-4px, -4px);
    }
    .error {
      margin-top: var(--space-lg);
      border: var(--border-thick);
      padding: var(--space-md);
      background: var(--red);
      color: var(--white);
      font-family: var(--font-mono);
      font-size: 1.1rem;
      font-weight: 700;
      text-transform: uppercase;
      display: none;
      box-shadow: var(--shadow-hard);
    }
    .error.show {
      display: block;
      animation: shake 0.3s ease-in-out;
    }
    @keyframes shake {
      0%, 100% { transform: translateX(0); }
      25% { transform: translateX(-10px); }
      75% { transform: translateX(10px); }
    }
    .qr-section {
      margin-top: var(--space-lg);
      border: var(--border-thick);
      padding: var(--space-md);
      background: var(--white);
      display: none;
    }
    .qr-section.show {
      display: block;
    }
    .qr-title {
      font-family: var(--font-display);
      font-size: 1.5rem;
      text-transform: uppercase;
      margin-bottom: var(--space-sm);
    }
    .qr-code {
      width: 200px;
      height: 200px;
      border: var(--border-thick);
      background: var(--white);
      display: block;
      margin: var(--space-md) 0;
    }
    .qr-code img {
      width: 100%;
      height: 100%;
      display: block;
    }
    .footer {
      border: var(--border-thick);
      background: var(--black);
      color: var(--white);
      padding: var(--space-md);
      text-transform: uppercase;
      font-family: var(--font-mono);
      font-size: 0.9rem;
      font-weight: 700;
      text-align: center;
    }
    .footer-links {
      margin-top: var(--space-md);
      display: flex;
      justify-content: center;
      gap: var(--space-md);
      flex-wrap: wrap;
    }
    .footer-link {
      color: var(--white);
      text-decoration: none;
      text-transform: uppercase;
      font-weight: 700;
      letter-spacing: 0.05em;
    }
    .footer-link:hover {
      text-decoration: underline;
      background: var(--white);
      color: var(--black);
    }
    .stats-section {
      margin-top: var(--space-lg);
      border: var(--border-thick);
      padding: var(--space-lg) var(--space-md);
      background: var(--white);
    }
    .stat-item {
      display: flex;
      justify-content: space-between;
      padding: var(--space-sm) 0;
      border-bottom: var(--border-thin);
      font-family: var(--font-mono);
      font-size: 1rem;
      text-transform: uppercase;
    }
    .stat-label {
      font-weight: 700;
    }
    .stat-value {
      font-weight: 400;
      color: var(--black);
    }
    .deco-line {
      width: 100%;
      height: var(--space-xs);
      background: var(--black);
      margin: var(--space-md) 0;
    }
    .deco-box {
      width: 100%;
      border: var(--border-thin);
      padding: var(--space-sm);
      margin: var(--space-md) 0;
      font-family: var(--font-mono);
      font-size: 0.8rem;
      text-transform: uppercase;
      font-weight: 700;
      background: var(--white);
    }
    .success-marker {
      display: inline-block;
      width: 20px;
      height: 20px;
      background: var(--black);
      margin-right: var(--space-sm);
      vertical-align: middle;
    }
    .success-marker::after {
      content: '✓';
      color: var(--white);
      display: block;
      text-align: center;
      line-height: 20px;
      font-size: 14px;
    }
    @media (max-width: 768px) {
      .header h1 {
        transform: rotate(0deg);
      }
      .result-actions {
        flex-direction: column;
      }
      .copy-btn, .visit-btn {
        width: 100%;
      }
    }
  </style>
</head>
<body>
  <script>
    // Check for saved variant preference
    (function() {
      const urlParams = new URLSearchParams(window.location.search);
      const currentVariant = urlParams.get('variant');
      if (!currentVariant) {
        const savedVariant = localStorage.getItem('shrtname_variant');
        if (savedVariant && savedVariant !== 'brutalist') {
          window.location.href = '/?variant=' + savedVariant;
        }
      } else {
        localStorage.setItem('shrtname_variant', currentVariant);
      }
    })();
  </script>
  <div class="container">
    <header class="header">
      <h1>SHRT.NAME</h1>
      <p>BRUTALIST URL SHORTENER</p>
      <div class="deco-line"></div>
      <p>NO DESIGN // JUST FUNCTION</p>
    </header>

    <main class="main">
      <h2 class="section-title">CREATE SHORT URL</h2>

      <form id="shortenForm">
        <div class="input-group">
          <label class="input-label" for="url">PASTE YOUR LONG URL</label>
          <div class="input-wrapper">
            <input type="url" id="url" class="url-input" placeholder="HTTPS://EXAMPLE.COM/VERY/LONG/URL" required autocomplete="off">
          </div>
        </div>

        <div class="custom-code-group">
          <label class="input-label" for="customCode" style="font-size: 1.3rem;">CUSTOM CODE [OPTIONAL]</label>
          <input type="text" id="customCode" class="custom-code-input" placeholder="ENTER CUSTOM CODE" maxlength="50" pattern="[a-zA-Z0-9-_]+" autocomplete="off">
        </div>

        <button type="submit" class="submit-btn">SHORTEN URL</button>
      </form>

      <div id="loading" class="loading" style="display: none;">PROCESSING...</div>

      <div id="error" class="error"></div>

      <div id="result" class="result">
        <h3 class="result-title"><span class="success-marker"></span>URL SHORTENED</h3>
        <div class="short-url" id="shortUrl"></div>
        <div class="original-url" id="originalUrl"></div>

        <div class="result-actions">
          <button id="copyBtn" class="copy-btn">COPY TO CLIPBOARD</button>
          <button id="visitBtn" class="visit-btn">OPEN IN NEW TAB</button>
        </div>
      </div>

      <div id="qrSection" class="qr-section">
        <h4 class="qr-title">QR CODE</h4>
        <div class="qr-code" id="qrCode"></div>
      </div>
    </main>

    <div id="statsSection" class="stats-section" style="display: none;">
      <h2 class="section-title">LINK STATISTICS</h2>
      <div class="stat-item">
        <span class="stat-label">TOTAL CLICKS</span>
        <span class="stat-value" id="clickCount">0</span>
      </div>
      <div class="stat-item">
        <span class="stat-label">CREATED</span>
        <span class="stat-value" id="createdAt">-</span>
      </div>
      <div class="stat-item">
        <span class="stat-label">CODE TYPE</span>
        <span class="stat-value" id="codeType">-</span>
      </div>
    </div>

    <div class="deco-box">// BRUTALIST DESIGN PRINCIPLE: FORM FOLLOWS FUNCTION //</div>

    <footer class="footer">
      <p>POWERED BY CLOUDFLARE WORKERS + D1 + KV</p>
      <div class="deco-line" style="background: var(--white);"></div>
      <p>$0/MONTH // FOREVER FREE</p>
      <div class="footer-links">
        <a href="/" class="footer-link">HOME</a>
        <a href="/?variant=brutalist" class="footer-link">BRUTALIST</a>
        <a href="/?variant=luxury" class="footer-link">LUXURY</a>
        <a href="/?variant=playful" class="footer-link">PLAYFUL</a>
        <a href="/?variant=industrial" class="footer-link">INDUSTRIAL</a>
      </div>
    </footer>
  </div>

  <script>
    const form = document.getElementById('shortenForm');
    const urlInput = document.getElementById('url');
    const customCodeInput = document.getElementById('customCode');
    const loading = document.getElementById('loading');
    const error = document.getElementById('error');
    const result = document.getElementById('result');
    const shortUrl = document.getElementById('shortUrl');
    const originalUrl = document.getElementById('originalUrl');
    const copyBtn = document.getElementById('copyBtn');
    const visitBtn = document.getElementById('visitBtn');
    const qrSection = document.getElementById('qrSection');
    const qrCode = document.getElementById('qrCode');
    const statsSection = document.getElementById('statsSection');
    const clickCount = document.getElementById('clickCount');
    const createdAt = document.getElementById('createdAt');
    const codeType = document.getElementById('codeType');
    const API_BASE = window.location.origin;

    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      const url = urlInput.value.trim();
      const customCode = customCodeInput.value.trim().toUpperCase();

      if (!url) {
        showError('URL IS REQUIRED');
        return;
      }
      if (!isValidUrl(url)) {
        showError('INVALID URL FORMAT');
        return;
      }
      hideError();
      hideResult();
      showLoading();

      try {
        const response = await fetch(\`\${API_BASE}/api/shorten\`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ url: url, code: customCode || undefined }),
        });
        const data = await response.json();
        if (response.ok) {
          showResult(data);
        } else {
          showError(data.error || 'FAILED TO SHORTEN URL');
        }
      } catch (err) {
        showError('NETWORK ERROR - TRY AGAIN');
        console.error(err);
      } finally {
        hideLoading();
      }
    });

    copyBtn.addEventListener('click', async () => {
      const url = shortUrl.textContent;
      try {
        await navigator.clipboard.writeText(url);
        copyBtn.textContent = 'COPIED!';
        setTimeout(() => { copyBtn.textContent = 'COPY TO CLIPBOARD'; }, 2000);
      } catch (err) {
        showError('FAILED TO COPY');
      }
    });

    visitBtn.addEventListener('click', () => {
      window.open(shortUrl.textContent, '_blank');
    });

    function isValidUrl(string) {
      try {
        const url = new URL(string);
        return url.protocol === 'http:' || url.protocol === 'https:';
      } catch (_) {
        return false;
      }
    }

    function showLoading() { loading.style.display = 'block'; }
    function hideLoading() { loading.style.display = 'none'; }
    function showError(message) {
      error.textContent = message;
      error.classList.add('show');
    }
    function hideError() {
      error.classList.remove('show');
      error.textContent = '';
    }
    function showResult(data) {
      shortUrl.textContent = data.short_url;
      originalUrl.textContent = data.original_url;
      result.classList.add('show');
      qrSection.classList.add('show');
      generateQRCode(data.short_url);
      fetchStats(data.code);
    }
    function hideResult() {
      result.classList.remove('show');
      qrSection.classList.remove('show');
      statsSection.style.display = 'none';
    }
    function generateQRCode(url) {
      const qrUrl = \`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=\${encodeURIComponent(url)}&bgcolor=ffffff&color=000000\`;
      qrCode.innerHTML = \`<img src="\${qrUrl}" alt="QR Code">\`;
    }
    async function fetchStats(code) {
      try {
        const response = await fetch(\`\${API_BASE}/api/stats/\${code}\`);
        const data = await response.json();
        if (response.ok) {
          statsSection.style.display = 'block';
          clickCount.textContent = data.clicks;
          createdAt.textContent = new Date(data.created_at).toLocaleDateString().toUpperCase();
          codeType.textContent = data.custom_code ? 'CUSTOM' : 'AUTO-GENERATED';
        }
      } catch (err) {
        console.error('Failed to fetch stats:', err);
      }
    }
    customCodeInput.addEventListener('keypress', (e) => {
      if (e.key === 'Enter') {
        e.preventDefault();
      }
    });
    urlInput.focus();
  </script>
</body>
</html>`;
